...
def generate_launch_description():
    declare_loaded_description = DeclareLaunchArgument(
        ...
        description='Set robot_description text.  \
                     It is recommended to use RobotDescriptionLoader() in rupe_description.'
    )

    declare_rviz_config_file = DeclareLaunchArgument(
        'rviz_config_file',
        default_value=get_package_share_directory(
            'rupe_moveit_config') + '/launch/run_move_group.rviz',
    ...
    robot_description_semantic_config = load_file(
        'rupe_moveit_config', 'config/rupe.srdf')
    robot_description_semantic = {
        'robot_description_semantic': robot_description_semantic_config}

    kinematics_yaml = load_yaml('rupe_moveit_config', 'config/kinematics.yaml')
    ...
    ompl_planning_yaml = load_yaml('rupe_moveit_config', 'config/ompl_planning.yaml')
    ompl_planning_pipeline_config['move_group'].update(ompl_planning_yaml)

    # Trajectory Execution Functionality
    controllers_yaml = load_yaml('rupe_moveit_config', 'config/controllers.yaml')
    moveit_controllers = {
