...
#include "rupe_control/rupe_hardware.hpp"
...
RupeHardware::~RupeHardware()
{
  driver_->torque_enable(false);
  driver_->close_port();
}

CallbackReturn RupeHardware::on_init(
  const hardware_interface::HardwareInfo & info)
{
  ...
  for (auto joint : info_.joints) {
    if (joint.parameters["dxl_id"] != "") {
      ...
      RCLCPP_ERROR(
        rclcpp::get_logger("RupeHardware"),
  ...
  // Open a rupe_driver
  driver_ = std::make_shared<RupeDriver>(port_name, baudrate, dxl_id_list);
  if (!driver_->open_port()) {
    RCLCPP_ERROR(
      rclcpp::get_logger("RupeHardware"), driver_->get_last_error_log().c_str());
    return CallbackReturn::ERROR;
  }
  ...
  // Verify that the interface required by RupeHardware is set in the URDF.
...
RupeHardware::export_state_interfaces()
{
...
RupeHardware::export_command_interfaces()
{
...
CallbackReturn RupeHardware::on_activate(const rclcpp_lifecycle::State & /*previous_state*/)
{
  if (!driver_->torque_enable(false)) {
    RCLCPP_ERROR(
      rclcpp::get_logger("RupeHardware"),
...
CallbackReturn RupeHardware::on_deactivate(
...
return_type RupeHardware::read(
  const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/)
{
  if (communication_timeout()) {
    if (!timeout_has_printed_) {
      RCLCPP_ERROR(
        rclcpp::get_logger("RupeHardware"), "Communication timeout!");
...
return_type RupeHardware::write(
  const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/)
{
  if (communication_timeout()) {
    if (!timeout_has_printed_) {
...
bool RupeHardware::communication_timeout()
{
